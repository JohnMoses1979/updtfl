...
//       loginType: data.loginType || "EMPLOYEE",
...
//   return { status: "success", message: "Face capture completed on device.", paths: null };
// }
...
// export async function apiLogin(arg1, arg2, arg3 = "EMPLOYEE", arg4 = {}) {
//   const payload =
//     typeof arg1 === "object" && arg1 !== null
//       ? arg1
//       : { empId: arg1, password: arg2, loginType: arg3, ...arg4 };
...
//       loginType: payload.loginType || "EMPLOYEE",
...
//   const endpoint = isClockedIn ? "/api/attendance/check-out" : "/api/attendance/check-in";
//   const attendanceRes = await fetch(`${BASE_URL}${endpoint}`, {
//     method: "POST",
//     headers: {
...
//   if (!attendanceRes.ok) {
//     return { match: false, score: 0, message: attendanceData.message || "Attendance request failed" };
//   }
//   return {
//     match: true,
//     score: 1,
//     message: isClockedIn ? "Check-out recorded" : "Check-in recorded",
...
// ── Step 1: Signup ──────────────────────────────────────────────────────────
// export async function apiSignup({ name, email, empId, designation, password, loginType = "EMPLOYEE" }) {
//   const res = await fetch(`${BASE_URL}${ENDPOINTS.signup}`, {
//     method: "POST",
//     headers: { "Content-Type": "application/json" },
//     body: JSON.stringify({ name, email, empId, designation, password, loginType }),
//   });
//   const data = await res.json();
...
export async function apiLogin(arg1, arg2, arg3 = "EMPLOYEE", arg4 = {}) {
  ...
  const data = await res.json();
  if (!res.ok) throw new Error(data.message || "Login failed");
  if (data.token) tokenStore.set(data.token);   // ← store JWT
  return data;
}
// ── Face Verify (check-in / check-out) ──────────────────────────────────────
export async function apiFaceVerify(empId, photoUri, isClockedIn = false) {
  ...
  /*
      ...
      const data = await res.json();
      console.log("[apiFaceVerify] Response:", data);
      // ✅ NEW: biometric → JWT login flow
      if (data.match && data.biometricToken) {
          try {
              const loginRes = await fetch(`${BASE_URL}/api/auth/biometric-login`, {
                  method: "POST",
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify({
                      empId,
                      verifyToken: data.biometricToken,
                  }),
              });
