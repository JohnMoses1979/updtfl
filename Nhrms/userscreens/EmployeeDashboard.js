...
/**
 * userscreens/EmployeeDashboard.js
 * API: GET /api/payroll/salary/by-emp/{empId}
 *      GET /api/payslip/by-emp/{empId}/{month}/{year}
 * Uses UserContext for user info
 * Navigates to: ViewPayslip, SalaryBreakdown, PayrollHistory
 */
import React, { useEffect, useState } from "react";
...
import { useUser } from "../context/UserContext";
import { fetchCurrentMonthSalary, fetchPayslipByEmp } from "../api/salaryApi";
...
export default function EmployeeDashboard({ navigation }) {
  const { user } = useUser();
  const [salary, setSalary] = useState(null);
  const [lastMonthPayslip, setLastMonthPayslip] = useState(null);
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    ...
    const loadDashboard = async () => {
      ...
      try {
        const [salaryData, payslipData] = await Promise.all([
          fetchCurrentMonthSalary(user.empId),
          fetchPayslipByEmp(user.empId, lastMonthDate.getMonth() + 1, lastMonthDate.getFullYear()).catch(() => null),
        ]);
        ...
        setLastMonthPayslip(
          payslipData && Number(payslipData.earnedSalary || 0) > 0 ? payslipData : null
        );
      } catch (err) {
        if (!active) return;
        console.error("[EmployeeDashboard] dashboard fetch error:", err.message);
        setSalary(null);
        setLastMonthPayslip(null);
      } finally {
  ...
  const earnedSalary = Number(salary?.earnedSalary || 0);
  const grossSalary = Number(salary?.grossSalary || 0);
  const presentDays = Number(salary?.presentDays || 0);
  const currentMonth = salary?.month ? MONTHS[salary.month - 1] : MONTHS[new Date().getMonth()];
  const currentYear = salary?.year || new Date().getFullYear();
  const lastMonthLabel = lastMonthPayslip
    ? `${String(lastMonthPayslip.month || "").charAt(0)}${String(lastMonthPayslip.month || "").slice(1).toLowerCase()} ${lastMonthPayslip.year}`
    : "Last Month";
 
  const recentPayslips = [
    {
      month: "This Month",
      generated: `${presentDays} checkouts recorded`,
      amount: fmtCurrency(earnedSalary),
      status: earnedSalary > 0 ? "Active" : "Pending",
    },
    {
      month: lastMonthLabel,
      generated: lastMonthPayslip
        ? `${lastMonthPayslip.presentDays || 0} checkouts recorded`
        : "Payroll not generated",
      amount: fmtCurrency(lastMonthPayslip?.netSalary || 0),
      status: lastMonthPayslip ? "Paid" : "Pending",
    },
  ];
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
        ...
        {loading ? (
          ...
          <View style={styles.salaryCard}>
            <Text style={styles.salaryLabel}>
              {currentMonth} {currentYear} Earnings
            </Text>
            <Text style={styles.salaryAmount}>{fmtCurrency(earnedSalary)}</Text>
            <Text style={styles.employeeInfo}>{displayDesignation} · {displayEmpId}</Text>
            <View style={styles.salaryStats}>
              <View style={styles.statItem}>
                <Text style={styles.statLabel}>Gross</Text>
                <Text style={styles.statValue}>{fmtCurrency(grossSalary)}</Text>
              </View>
              <View style={styles.statDivider} />
              <View style={styles.statItem}>
              ...
              <View style={styles.statDivider} />
              <View style={styles.statItem}>
                <Text style={styles.statLabel}>Per Checkout</Text>
                <Text style={[styles.statValue, styles.overtimeValue]}>₹612</Text>
              </View>
            </View>
          </View>
        )}
        ...
        <View style={styles.section}>
          ...
          {recentPayslips.map((slip, index) => (
            <View key={index} style={styles.payslipItem}>
              ...
              <View style={styles.payslipRight}>
                <Text style={styles.payslipAmount}>{slip.amount}</Text>
                <View style={[styles.paidBadge, !["Paid", "Active"].includes(slip.status) && styles.pendingBadge]}>
                  <Text style={[styles.paidText, !["Paid", "Active"].includes(slip.status) && styles.pendingText]}>
                    {slip.status}
                  </Text>
                </View>
              </View>
