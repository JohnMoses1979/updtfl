...
import { useUser } from "../context/UserContext";
import { fetchPayslipByEmp } from "../api/salaryApi";
...
export default function SalaryBreakdown({ navigation }) {
  const { user } = useUser();
  ...
  useEffect(() => {
    ...
    const loadPayslip = async () => {
      ...
      try {
        const json = await fetchPayslipByEmp(user.empId, month, year);
        if (!active) return;
 
        if (!json || Number(json.earnedSalary || 0) === 0) {
          setData(null);
          setError(json?.message || "No payslip data for last month.");
        } else {
          setData(json);
  ...
  const earned = Number(data.earnedSalary || 0);
  const basic = Number(data.basicSalary || 0);
  const hra = Number(data.hra || 0);
  const transport = Number(data.transport || 0);
  const specialAllow = Number(data.specialAllowance || 0);
  const tax = Number(data.tax || 0);
  const insurance = Number(data.insurance || 0);
  const totalDeductions = Number(data.totalDeductions || (tax + insurance));
  const netSalary = Number(data.netSalary || Math.max(0, earned - totalDeductions));
  const presentDays = Number(data.presentDays || 0);
  const perDayRate = Number(data.perDayRate || 612);
  const monthLabel = `${MONTHS_FULL[month - 1]} ${year}`;
