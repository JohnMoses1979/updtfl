...
/**
 * userscreens/ViewPayslip.js
 * API: GET /api/payslip/by-emp/{empId}/{month}/{year}
 * Uses UserContext for empId
 */
import React, { useState, useEffect } from "react";
import {
  View, Text, ScrollView, TouchableOpacity,
...
import { Platform, Linking } from "react-native";
import { useUser } from "../context/UserContext";
import { buildPayslipDownloadUrl, fetchPayslipByEmp } from "../api/salaryApi";
...
export default function ViewPayslip({ navigation }) {
  const { user } = useUser();
  const displayName = user?.name || "Employee";
  const displayEmpId = user?.empId || "—";
  const displayDesignation = user?.designation || "Employee";
  ...
  useEffect(() => {
    ...
    const loadPayslip = async () => {
      ...
      try {
        const data = await fetchPayslipByEmp(user.empId, selected.month, selected.year);
        if (!active) return;
 
        if (!data || Number(data.earnedSalary || 0) === 0) {
          setError(data?.message || "No attendance data for this month.");
        } else {
          setPayslip(data);
        }
  ...
  const handleDownloadPdf = async () => {
    if (!selected || !payslip || !user?.empId) {
      Alert.alert("No Payslip", "Please select a month with payslip data first.");
      return;
    }
 
    const url = buildPayslipDownloadUrl(user.empId, selected.month, selected.year);
    ...
    if (Platform.OS === "web") {
      ...
      try {
        ...
        if (supported) {
          ...
          Alert.alert("Cannot Open", "Could not open the PDF URL. Please try on a browser.");
        }
      } catch (err) {
        Alert.alert("Download Failed", err.message || "Could not open PDF.");
      }
        ...
        {!loading && payslip && (
          <>
            <View style={styles.slipCard}>
              ...
              {Number(payslip.grossSalary || 0) > 0 && (
                <View style={styles.lineItem}>
                  <Text style={styles.lineItemLabel}>Monthly Gross (CTC)</Text>
                  <Text style={styles.lineItemAmount}>{fmt(payslip.grossSalary)}</Text>
                </View>
...
const styles = StyleSheet.create({
  ...
  sectionLabel: { marginBottom: 8 },
  sectionLabelText: { fontSize: 13, color: "#9ca3af", fontWeight: "500" },
  monthScroll: { marginBottom: 16 },
  monthChip: {
    backgroundColor: "#1a3a5c", borderRadius: 18, paddingHorizontal: 16, paddingVertical: 8, marginRight: 8,
    borderWidth: 1, borderColor: "transparent",
  },
  monthChipActive: { borderColor: "#2F6E8E", backgroundColor: "#17314f" },
  monthChipText: { color: "#c7cddb", fontSize: 13, fontWeight: "600" },
  monthChipTextActive: { color: "#ffffff" },
  centered: { alignItems: "center", paddingTop: 40, gap: 12 },
  loadingText: { color: "#9ca3af", fontSize: 14 },
  errorBox: {
    backgroundColor: "#1a2334", borderRadius: 18, padding: 24, alignItems: "center",
    borderWidth: 1, borderColor: "#23324b",
  },
  errorIcon: { fontSize: 38, marginBottom: 8 },
  errorText: { color: "#9ca3af", textAlign: "center", fontSize: 14 },
  slipCard: {
  ...
  slipTitle: { fontSize: 22, fontWeight: "800", color: "#111827" },
  slipSubtitle: { fontSize: 13, color: "#6b7280", marginTop: 4 },
  presentBadge: {
    backgroundColor: "#ecfdf5", borderRadius: 999, paddingHorizontal: 12, paddingVertical: 6,
  },
  presentBadgeText: { color: "#16a34a", fontWeight: "700", fontSize: 12 },
  employeeGrid: { flexDirection: "row", flexWrap: "wrap", marginHorizontal: -6 },
  employeeField: { width: "50%", paddingHorizontal: 6, marginBottom: 14 },
  fieldLabel: {
    fontSize: 11, color: "#9ca3af", fontWeight: "700", marginBottom: 4, textTransform: "uppercase",
  },
  fieldValue: { fontSize: 13, fontWeight: "600", color: "#111827" },
  divider: { height: 1, backgroundColor: "#e5e7eb", marginVertical: 18 },
  categoryTitle: { fontSize: 12, fontWeight: "800", color: "#2F6E8E", marginBottom: 12, letterSpacing: 0.4 },
  deductionTitle: { color: "#dc2626" },
  lineItem: {
    flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingVertical: 7,
  },
  lineItemLabel: { fontSize: 14, color: "#374151" },
  lineItemAmount: { fontSize: 14, fontWeight: "600", color: "#111827" },
  deductionAmount: { color: "#dc2626" },
  totalRow: {
    flexDirection: "row", justifyContent: "space-between", alignItems: "center",
    paddingTop: 12, marginTop: 8, borderTopWidth: 1, borderTopColor: "#e5e7eb",
  },
  totalLabel: { fontSize: 15, fontWeight: "700", color: "#111827" },
  ...
  netSalaryLabel: { fontSize: 15, fontWeight: "700", color: "#166534" },
  netSalaryAmount: { fontSize: 22, fontWeight: "800", color: "#111827" },
  downloadButton: {
    marginTop: 18, backgroundColor: "#2F6E8E", borderRadius: 16, paddingVertical: 16,
