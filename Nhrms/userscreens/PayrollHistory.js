...
export default function PayrollHistory({ navigation }) {
  const { user } = useUser();
  const displayName = user?.name || "Employee";
  const displayEmpId = user?.empId || "—";
  const displayDesignation = user?.designation || "Employee";
  ...
  const fetchAllHistory = useCallback(async () => {
    ...
    await Promise.all(
      allMonths.map(async ({ month, year }) => {
        try {
          const data = await fetchPayslipByEmp(user.empId, month, year);
          if (data && Number(data.earnedSalary || 0) > 0) {
            results.push({
              ...
              year: String(year),
              monthName: MONTHS[month - 1],
              gross: Number(data.earnedSalary || 0),
  ...
  const years = [...new Set(historyData.map((r) => r.year))].sort((a, b) => Number(b) - Number(a));
  useEffect(() => {
    if (years.length > 0 && !years.includes(selectedYear)) {
      setSelectedYear(years[0]);
    }
