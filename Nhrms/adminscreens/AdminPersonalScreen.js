...
//                 JPG or PNG, minimum 800×800px, max 5MB
//             </Text>
//         </View>
//     );
// }
...
//         if (pickerType === "city" && !state) {
//             Alert.alert("Select state", "Please choose a state first.");
//             return;
//         }
//         setActiveLocationPicker(pickerType);
//     };
...
//                     { paddingBottom: isSmallPhone ? 120 : 130 },
//                 ]}
...
//                     sub="Details about my personal data"
//                 />
//                 <AvatarSection
//                     firstName={firstName}
//                     lastName={lastName}
...
//                     onPress={() => Alert.alert("Designation", "Integrate designation picker here.")}
//                 />
...
/**
 * adminscreens/AdminPersonalScreen.js
 *
 * FIXES:
 *  1. After successful save, calls updateProfile() on UserContext so
 *     admin name persists across navigation/reload.
 *  2. Initialises from UserContext data when initialData is sparse.
...
const INDIA_LOCATION_OPTIONS = {
    India: {
        ...
        Telangana: ["Hyderabad", "Warangal", "Karimnagar", "Nizamabad", "Khammam", "Ramagundam", "Mahabubnagar", "Nalgonda", "Adilabad", "Suryapet"],
        Karnataka: ["Bangalore", "Mysore", "Hubli", "Mangalore", "Belgaum", "Davangere", "Bellary", "Bijapur", "Shimoga", "Gulbarga"],
        ...
        Delhi: ["New Delhi", "Dwarka", "Rohini", "Janakpuri", "Saket", "Lajpat Nagar", "Karol Bagh", "Connaught Place", "Vasant Kunj", "Pitampura"],
        Gujarat: ["Ahmedabad", "Surat", "Vadodara", "Rajkot", "Bhavnagar", "Jamnagar", "Junagadh", "Gandhinagar", "Anand", "Morbi"],
        Rajasthan: ["Jaipur", "Jodhpur", "Kota", "Bikaner", "Ajmer", "Udaipur", "Bhilwara", "Alwar", "Bharatpur", "Sikar"],
        "Uttar Pradesh": ["Lucknow", "Kanpur", "Agra", "Varanasi", "Meerut", "Prayagraj", "Ghaziabad", "Noida", "Firozabad", "Bareilly"],
...
const C = {
    ...
    gray: "#AAAAAA", graySoft: "#6E6E6E", darkCard: "#0f1e30",
    border: "#1a3a5c", inputBg: "#1a2f4a", avatarBg: "#163352",
};
        ...
        <View style={styles.fieldWrap}>
            ...
            <View style={[styles.fieldBox, multiline && styles.fieldBoxMultiline]}>
                ...
                <TextInput
                    ...
                    placeholderTextColor="#555" multiline={multiline} keyboardType={keyboardType}
                />
            </View>
        </View>
    );
...
export default function AdminPersonalScreen({ onBack, onSave, initialData = {} }) {
    ...
    // ── Initialise from initialData or UserContext ─────────────
    ...
    useEffect(() => {
        if (!INDIA_LOCATION_OPTIONS[country]) { setCountry("India"); setState(""); setCity(""); }
    }, [country]);
    useEffect(() => {
        if (state && !stateOptions.includes(state)) { setState(""); setCity(""); }
    }, [state, stateOptions]);
    useEffect(() => {
    ...
    const handleUpdate = async () => {
        ...
        const payload = {
            ...
            dob, designation: designation.trim(),
            country, state, city,
        ...
        try {
            setSaving(true);
            // Admin profile: save via empId if available, otherwise just update context
            const empId = initialData?.empId || user?.empId;
            if (empId) {
                const res = await fetch(`${BASE_URL}/api/employees/profile/${empId}`, {
                    method: "PUT",
                    headers: { "Content-Type": "application/json" },
    ...
    const handleRemoveImage = () => setAvatarUri("");
    ...
    return (
        <SafeAreaView style={styles.safe}>
            ...
            <ScrollView
                ...
                keyboardShouldPersistTaps="handled"
            >
                <SectionLabel title="My Personal Data" sub="Details about my personal data" />
                <AvatarSection
                    firstName={firstName} lastName={lastName} avatarUri={avatarUri}
                ...
                <DropdownField label="Date of Birth" icon="📅" value={dob} placeholder="Select date of birth" onPress={() => setDobPickerVisible(true)} />
                <DropdownField label="Designation" icon="🪪" value={designation} placeholder="Select designation" onPress={() => Alert.alert("Designation", "Integrate designation picker here.")} />
                <View style={styles.addressSection}>
                    <Text style={styles.addressTitle}>Address</Text>
                    <Text style={styles.addressSub}>Your current domicile</Text>
                </View>
            ...
            <View style={styles.updateWrap}>
                <TouchableOpacity
                    ...
                    onPress={handleUpdate} activeOpacity={0.85} disabled={saving}
                >
            ...
            <AppDatePickerModal
                visible={dobPickerVisible} value={dob}
                onClose={() => setDobPickerVisible(false)} onChange={setDob}
            />
            <AppOptionPickerModal visible={activeLocationPicker === "country"} title="Select Country" options={countryOptions} selectedValue={country} onSelect={(v) => { setCountry(v); setState(""); setCity(""); setActiveLocationPicker(null); }} onClose={() => setActiveLocationPicker(null)} />
            <AppOptionPickerModal visible={activeLocationPicker === "state"} title="Select State" options={stateOptions} selectedValue={state} onSelect={(v) => { setState(v); setCity(""); setActiveLocationPicker(null); }} onClose={() => setActiveLocationPicker(null)} />
            <AppOptionPickerModal visible={activeLocationPicker === "city"} title="Select City" options={cityOptions} selectedValue={city} onSelect={(v) => { setCity(v); setActiveLocationPicker(null); }} onClose={() => setActiveLocationPicker(null)} />
        </SafeAreaView>
    );
}
...
const styles = StyleSheet.create({
    ...
    topNav: { flexDirection: "row", alignItems: "center", paddingHorizontal: 16, paddingTop: Platform.OS === "android" ? (StatusBar.currentHeight || 0) + 10 : 10, paddingBottom: 14, borderBottomWidth: 1, borderBottomColor: C.border },
    ...
    cameraText: { color: C.white, fontSize: 14, fontWeight: "700" },
